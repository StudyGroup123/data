...
if data_name=='suochi_0.3_20':
    timesteps = 10  #时序是20频域是40
    num_classes = 4
    data = np.loadtxt('/home/bridge3/lstnet/esncnn/data/0.15kg_4li/0.15kg_4li-10/0.15kg_4li-10-data.txt')
    label =np.loadtxt('/home/bridge3/lstnet/esncnn/data/0.15kg_4li/0.15kg_4li-10/0.15kg_4li-10-label.txt')
...
# 0layer
dense=Dense(512,activation='relu')(input)
...
#optimizer = SGD(lr=0.01, momentum=0.9) #re->0.90
...
#||||||||||||SAME|||||||||||||||
callbacks=[
    EarlyStopping(monitor='val_loss', patience=200, verbose=1, mode='auto'),
    ModelCheckpoint('./'+data_name+'/'+data_name+'_model'+r+'.h5',monitor = 'val_acc',save_best_only=True),
]  
history=model.fit(x_train,y_train,batch_size=bantch,epochs=200,validation_split=0.33,callbacks=callbacks,shuffle=True)
